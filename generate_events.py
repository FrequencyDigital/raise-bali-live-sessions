#!/usr/bin/env python3
"""Generate up to 200 event-nomination fixtures for the SEE Awards form."""
        ...
        records.append(
            {
                ...
                "most_innovative": rng.choice(EVENT_VARIANTS),
                "top_event": rng.choice(EVENT_VARIANTS),
                "status": "pending",
            }
        )
    return records
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate event nomination JSON fixtures (max 200 records)."
    )
    parser.add_argument(
        "--count",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=2027,
        help="RNG seed for reproducible output (default 2027).",
    )
    parser.add_argument(
        "--out",
        type=Path,
