#!/usr/bin/env python3
"""Submit SEE Awards nominations from JSON using Selenium.

Locks each record in nominations.json while it is in progress and after
it succeeds so it is not submitted again. Smoke-tests the first unlocked
record, then runs remaining records in a bounded parallel Chrome pool.
"""
...
PROMOTER_ID = "inp-promoter_year"
SOCIAL_ID = "inp-social_media_star"
...
def wait_for_nomination_post(driver: webdriver.Chrome, timeout: int) -> None:
        ...
        return bool(
            d.execute_script(
                ...
                try {
                    ...
                    // Cross-origin after Apps Script responds — POST was sent.
                    return true;
                }
                """
            )
    ...
    try:
        ...
        # Still give the request a moment before Chrome is closed.
        time.sleep(3)
...
def fill_and_submit(driver: webdriver.Chrome, record: dict, timeout: int) -> None:
    ...
    fill_input(driver, PROMOTER_ID, record["promoter_of_the_year"], timeout)
    fill_input(driver, SOCIAL_ID, record["social_media_star"], timeout)
...
class NominationStore:
    """Thread-safe nominations.json with per-record lock/status fields."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fill and submit SEE Awards nominations from JSON. "
            "Locks each record in nominations.json as in_progress, then ok. "
            "Already locked or completed records are skipped. "
            "Runs one smoke-test first, then remaining records in parallel."
        )
    )
    parser.add_argument(
        "--json",
        ...
        help="Path to nominations JSON (default nominations.json).",
    )
    parser.add_argument(
        "--workers",
        type=int,
    ...
    parser.add_argument(
        ...
        help="Where to write per-record results (default results.json).",
    )
    parser.add_argument(
        "--limit",
        type=int,
    ...
    parser.add_argument(
        "--reset-locks",
        action="store_true",
        help="Set every record in nominations.json back to pending and start over.",
    )
